 ...
 */
void a_assign( NUMBER *d, NUMBER *s )
{
	int l;
 ...
 */
void a_add( NUMBER *s1, NUMBER *s2, NUMBER *d )
{
	int l,lo,ld,same;
	register LONG sum;
 ...
 * lo== Laenge p3
 */
int n_sub( INT *p1, INT *p2, INT *p3, int l, int lo )
{
	int ld,lc,same;
	int over = 0;
	register LONG dif;
 ...
 */
void a_sub( NUMBER *s1, NUMBER *s2, NUMBER *d )
{
 ...
 * return neue Laenge
 */
int n_mult(INT *n, INT m, INT *d, int l)
{
	int i;
	register LONG mul;
 ...
 */
void a_imult( NUMBER *n, INT m, NUMBER *d)
{
	if (m == 0)
		d->n_len=0;
	else if (m == 1)
 ...
 */
void a_mult( NUMBER *m1, NUMBER *m2, NUMBER *d )
{
 ...
 * q = 0 : kein Quotient
 */
void n_div( NUMBER *d1, NUMBER *z2, NUMBER *q, NUMBER *r )
{
 ...
 * q = 0 : kein Quotient
 */
void a_div( NUMBER *d1, NUMBER *d2, NUMBER *q, NUMBER *r )
{
#if MAXINT != 1
	NUMBER z2[MAXBIT];
	INT z;
...
void m_add( NUMBER *s1, NUMBER *s2, NUMBER *d )
{
	a_add( s1, s2, d );
	if (a_cmp( d, mod_z2 ) >= 0)
		a_sub( d, mod_z2, d );
...
void m_mult( NUMBER *m1, NUMBER *m2, NUMBER *d )
{
	a_mult( m1, m2, d );
 ...
 */
void m_exp( NUMBER *x, NUMBER *n, NUMBER *z )
{
	NUMBER xt,nt;
 ...
 * GGT
 */
void a_ggt( NUMBER *a, NUMBER *b, NUMBER *f )
{
	NUMBER t[2];
	int at,bt, tmp;

 ...
 * 프로그램 개요 : Timer Class
 * 파일 위치     : BASE_DIR/source/system/common/
 *
 * 프로그램 추가 및 변경사항:
 *  1. 기존 원본 소스를 그대로 사용
 */
...
#define MAXEncLEN(l)	((l+18)*2)
#define MAXDecLEN(l)	((l/2)-2)

  /* 서재일 추가:
	ENDIAN을 precomplier(#ifndef BIGENDIAN)으로 하는것이 아니라
	런타임에 판단함
  */
#define _big_endian		0
#define _little_endian	1
static int endian = _little_endian;
...
/********************* Type Definitions **********************/
	...
	#else
		/* by lyg bus error 10 typedef unsigned long int	DWORD; */
		typedef unsigned int		DWORD;
...
void SeedEncrypt(BYTE *pbData, DWORD *pdwRoundKey)
{
	...
	R1 = ((DWORD *)pbData)[3];
/*#ifndef BIG_ENDIAN */
if(endian == _big_endian) {	/* 위의 define과는 반대임(서재일)	 */
	L0 = EndianChange(L0);
	L1 = EndianChange(L1);
	R0 = EndianChange(R0);
	R1 = EndianChange(R1);
  ...
  */
  {
	...
	unsigned char uc[2];
	memcpy(uc, &us, 2);
	if(uc[0] == 0x12) endian = _big_endian;
	else			  endian = _little_endian;
  }
  A = ((DWORD *)pbUserKey)[0];
  B = ((DWORD *)pbUserKey)[1];
  C = ((DWORD *)pbUserKey)[2];
...
static DWORD dwRoundKey[NoRoundKeys];
static int isMakeRoundKey=0;
static BYTE bUserKey[16] = { 0x06, 0x06, 0x00, 0x05, 0x01, 0x06, 0x01, 0x06,
							 0x07, 0x05, 0x07, 0x01, 0x03, 0x01, 0x02, 0x03 };
int SeedEnc(char *dest, char *sour, int sour_len)
{
	int i;
	int enc_len = ((sour_len/16)+1)*16;
	int n = enc_len / 16;
	BYTE *encData = (BYTE *)malloc(enc_len+4);
	BYTE *wrkData = (BYTE *)malloc(enc_len);
	BYTE h4;
	BYTE l4;
	BYTE *pt;
	int mock, na;

/* 	if(!isMakeRoundKey) { */	/* 라운드 키를 생성한다 */
/*		isMakeRoundKey=1; */
		SeedEncRoundKey(dwRoundKey, bUserKey);
/* 	} */
	memset(wrkData, 0, enc_len);
	memcpy(wrkData, sour, sour_len);
	pt = wrkData;
	for(i=0; i<n; i++) {	/* 16Byte씩 암호화 수행 */
		SeedEncrypt(pt, dwRoundKey);
		pt += 16;
	}
	memset(encData, 0, enc_len+4);

	mock = sour_len/(256*256*256);
	na = sour_len%(256*256*256);
	encData[0] = (BYTE) mock;  		/* 원래의 데이타길이 4번째 */

	mock = na/(256*256);
	na = na%(256*256);
	encData[1] = (BYTE) mock;  		/* 원래의 데이타길이 3번째 */

	mock = na/256;
	na = na%256;
	encData[2] = (BYTE) mock;  		/* 원래의 데이타길이 2번째 */

	encData[3] = (BYTE) na;	/* 원래의 데이타길이 1번째 */

	memcpy(&encData[4], wrkData, enc_len);

	for(i=0; i<enc_len+4; i++) {		/* 예) 0x12 -> '1' + '2'로 */
		h4 = (BYTE)(encData[i] / 16);
		l4 = (BYTE)(encData[i] % 16);
		if(h4 <= 9) *dest++ = h4 + '0';
		else		*dest++ = h4 - 10 + 'A';
		if(l4 <= 9) *dest++ = l4 + '0';
		else		*dest++ = l4 - 10 + 'A';
...
int SeedDec(char *dest, char *sour,int maxlen)
{
	...
	int enc_len = sour_len/2;
	BYTE *decData = (BYTE *)malloc(enc_len+16);
	BYTE *pt;
	BYTE h4;
	BYTE l4;
	int org_len;
...
/*	} */
	...
	pt = decData;
	for(i=0; i<sour_len; i+=2) { /* 예)) '1' + '2' 를 0x12로 */
		h4 = (BYTE) sour[i];
		l4 = (BYTE) sour[i+1];
		if(h4 <= '9') h4 = h4 - '0';
		else		  h4 = h4 - 'A' + 10;
	...
	org_len = (unsigned int) decData[0] * 256*256*256;
	org_len += (unsigned int) decData[1] * 256*256;
	org_len += (unsigned int) decData[2] * 256;
	org_len += (unsigned int) decData[3];
	if( org_len>maxlen || org_len==0 ) return -1;

	memcpy(decData, &decData[4], enc_len-4);	/* 길이를 뺀 원래 데이타를 복사 */
	...
	pt = decData;
	for(i=0; i<n; i++) {
		SeedDecrypt(pt, dwRoundKey);
		pt += 16;
	}
	memcpy(dest, decData, org_len);
	dest[org_len]=0;
...
{
	memset(bUserKey, 0, sizeof(bUserKey));
	memcpy(bUserKey, pKey, sizeof(bUserKey));
}
...
#ifdef BIG_ENDIAN
printf("realy defined BIG_ENDIAN\n");
#endif
#ifndef BIG_ENDIAN
printf("not defined BIG_ENDIAN\n");
#endif

/*
	BYTE bUserKey[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
							 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
	BYTE bData[16] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
						0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
	BYTE bUserKey[16] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
						0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
	BYTE bUserKey[16] = { 0x03, 0x02, 0x01, 0x00, 0x07, 0x06, 0x05, 0x04,
						0x0b, 0x0a, 0x09, 0x08, 0x0f, 0x0e, 0x0d, 0x0c };
	BYTE bUserKey[16] = { 0x0f, 0x0e, 0x0d, 0x0c, 0x0b, 0x0a, 0x09, 0x08,
						0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, 0x00 };
*/
	BYTE bUserKey[16] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
						0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
	BYTE bData[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
							 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
if(endian == _big_endian)
	printf("this machine's cpu is BIG_ENDIAN\r\n");
else
	printf("this machine's cpu is LITTLE_ENDIAN\r\n");


	for(i=0; i<NoRoundKeys; i++)
		printf("[%08x]",dwRoundKey[i]);
	printf("\n");

	SeedEncrypt(bData, dwRoundKey);

	for(i=0; i<16; i++)
		printf("[%02x]",bData[i]);
	printf("\n");

	SeedDecrypt(bData, dwRoundKey);
	...
	char *sour = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
	char dest[256];
	char org[256];
	memset(dest, 0, sizeof(dest));
	int ret = SeedEnc(dest, sour, strlen(sour));
	printf("ret[%d]",ret);
	...
	ret = SeedDec(org, dest, sizeof(dest));
	printf("ret[%d]",ret);
	printf("org[%s]",org);
}
#endif

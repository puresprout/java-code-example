 ...
 * 양력 음력 변환 클래스
 * 
 * 출처: http://50001.com/tt/board/ttboard.cgi?act=read&db=10102&sortby=title&order=asc&page=40&idx=682
 *    
 */
...
public class LunarCheck {
	int DayOfMonth[] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 }; // 양력
	// 월별
	// 일수
	int LunarDataNumberDay[] = { 29, 30, 58, 59, 59, 60 }; // 인덱스 숫자의 합

	// 1 : 작(29일), 2 : 큰(30일), 3 : 작작(윤달 - 29일,29일), 4 : 작큰(윤달 - 29일,30일), 5 :
	// 큰작(윤달 - 30일,29일), 6 : 큰큰(윤달 - 30일,30일)
	...
	public long countLunarDay(int Year, int Month, int Day, boolean Leap) {
		...
		if (Year >= 0) {
			...
			if (!Leap) {
				...
				if (LunarData[Year][Month - 1] == 1
						|| LunarData[Year][Month - 1] == 2) {
					AllCount += Day;
				} else if (LunarData[Year][Month - 1] == 3
						|| LunarData[Year][Month - 1] == 4) {
					AllCount += Day + 29;
				} else if (LunarData[Year][Month - 1] == 5
						|| LunarData[Year][Month - 1] == 6) {
					AllCount += Day + 30;
				}
			}
	...
	public String countToDateForLunar(long AllCountDay) {
		...
		String returnLunar = Long.toString(Year + 1900) + "/"
				+ nf.format(Integer.parseInt(Long.toString(Month))) + "/"
				+ nf.format(Integer.parseInt(Long.toString(Day)));
	...
	public String countToDateForSolar(long AllCountDay) {
		...
		String returnLunar = Long.toString(Year) + "/"
				+ nf.format(Integer.parseInt(Long.toString(Month))) + "/"
				+ nf.format(Integer.parseInt(Long.toString(AllCountDay)));
		// System.out.println("음력일자 : " + returnLunar);
		return returnLunar;
	}
